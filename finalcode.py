...
# function to draw on the canvas and overlay
...
# kernel definition for morphological transformations
...
# setup paint window
paintWindow = np.ones((471, 636, 3), dtype=np.uint8) * 255 #height width and the number of elements per field
paintWindow = cv2.rectangle(paintWindow, (40, 1), (140, 65), (0, 0, 0), 2) #the image being used is the paint window, 
#the rectangle corner points are also specified along with the color and the border thickness
...
while True:
    ...
    # horizontal flipping to make mirror image
    frame = cv2.flip(frame, 1)

    # resize the paint window to match the frame size
    paint_resized = cv2.resize(paintWindow, (frame.shape[1], frame.shape[0]))

    # convert frame to HSV
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    # get realtime trackbar values
    ...
    # define hsv range for the marker
    upper_hsv = np.array([u_hue, u_saturation, u_value])
    lower_hsv = np.array([l_hue, l_saturation, l_value])

    # create mask
    mask = cv2.inRange(hsv, lower_hsv, upper_hsv)#mask the hsv values to be in blue color range
    mask = cv2.erode(mask, kernel, iterations=1) 
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)
    mask = cv2.dilate(mask, kernel, iterations=1) #perform morphological transforms
